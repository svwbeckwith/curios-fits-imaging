...
def test_peak_cutout_clips_arbitrary_edge_location():
    image = np.zeros((100, 200))

    cutout = gui.peak_cutout(image, (199.0, 99.0), cutout_size=81)

    assert cutout["x1"] == 200
    assert cutout["y1"] == 100
    assert cutout["image"].size > 0
